...
class TeaOrderServiceTest {
    ...
    void findPageByOrderId() {
        teaOrderService.findPageByOrderId(1, "id", "ASC", 1L);
        Sort sort = Sort.by(Sort.Direction.valueOf("ASC"), "id");
        Pageable pageable = PageRequest.of(0, 10, sort);
        Mockito.verify(teaOrderRepository, Mockito.times(1)).findAllByOrderId(1L, pageable);
    }
    ...
    void save() {
        ...
        TeaOrder productOrder = orderFactory.getTeaOrder();

        Mockito.doReturn(Optional.of(new Order()))
                .when(teaOrderRepository)
                .findById(1L);

        TeaOrder checkedProductOrder = teaOrderService.save(productOrder);

        Assertions.assertEquals(checkedProductOrder, productOrder);
        Mockito.verify(teaOrderRepository, Mockito.times(1)).save(checkedProductOrder);
    }
    ...
    void update() {
        ...
        Assertions.assertEquals(checkedProductOrder, productOrder);
        Mockito.verify(teaOrderRepository, Mockito.times(1)).save(productOrder);
    }

    @Test
    void findById() {
        Mockito.doReturn(Optional.of(new TeaOrder()))
                .when(teaOrderRepository)
                .findById(1L);
        teaOrderService.findById(1L);
        Mockito.verify(teaOrderRepository, Mockito.times(1)).findById(1L);
    }

    @Test
    void deleteById() {
        teaOrderService.deleteById(1L);
        Mockito.verify(teaOrderRepository, Mockito.times(1)).deleteById(1L);
    }
}